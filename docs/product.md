# Redbot Product Plan

Redbot is a focused personal AI execution assistant. The first release avoids a blank-chat experience and starts users from concrete deliverables that are easy to show in a short video and easy to reuse in daily work.

## Target Users

- Creators who need research, scripts, and publishable material.
- Operators who write weekly reports, summaries, and content tables.
- Developers who want a tiny OpenAI-compatible agent template they can fork.

## MVP Workflows

1. **Research Brief**: turn a broad topic into angles, risks, and next actions.
2. **Short Video Script**: turn a technical idea into a 60-90 second script.
3. **Content Table**: turn notes into a structured comparison table.
4. **Weekly Report**: turn progress notes into a clean team update.
5. **GitHub README**: turn a product idea into an attractive open-source README.
6. **Local Client**: run Redbot on your own machine with `redbot desktop`, a browser console, webhook channels, and a status file containing local URLs.
7. **Memory + Knowledge Base**: remember per-user preferences, import local `.md/.txt` files, and retrieve local knowledge before running tasks.

## Differentiation

Compared with broad agent frameworks, Redbot is deliberately narrow:

- templates are the product surface;
- artifacts and traces are local, inspectable files;
- OpenAI-compatible configuration works with official APIs, gateways, and local model servers;
- the codebase is small enough for vibecoding tutorials and community remixing.

Compared with closed AI writing tools, Redbot is:

- MIT licensed;
- private by default when self-hosted;
- model-provider neutral;
- easy to extend with new workflow templates.

## Roadmap

- Template marketplace directory.
- Web UI for choosing templates and viewing traces.
- Richer desktop shell for the local client.
- Built-in web research tool.
- File ingestion for PDF, DOCX, and meeting notes.
- Scheduled recurring tasks.
- Chinese creator platform presets for Douyin, Xiaohongshu, Bilibili, and WeChat Official Accounts.
